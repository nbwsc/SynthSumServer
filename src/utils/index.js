...
async function downloadRequestPromise(data, task_id, id) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      //  })
      exeCommand(
        `curl -A "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/116.0.0.0 Safari/537.36" --insecure --connect-timeout 60 --max-time 36000 --limit-rate 20m "${data.href}" -L -o ${filename}`
      )
        .catch(() => {})
        .then(async (r) => {
          resolve({ ...data, pathname: filename });
